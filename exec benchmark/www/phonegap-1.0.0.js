...
PhoneGap = {
    ...
    commandQueue: [],
    // This queue is the same as commandQueue, but for PhoneGap.fastExec()
    // commands.
    fastExecCommandQueue: [],
    // Indicates if we're currently in the middle of flushing the fast exec
    // queue on the native side.
    fastExecFlushing: false,
    _constructors: [],
...
/** 
 * Execute a PhoneGap command, possibly queuing it up instead of executing it
 * immediately if another command has run previously but has not yet been
 * acknowledged by the native code. This ensures command execution happens
 * without any race conditions.  The risk is if we set the iframe src twice
 * (each signifying a command) before the native code gets notified of the
 * change. If this happens, the first command will never get run. 
 *
 * Simple parameters are passed as arguments on the url. JavaScript objects are
 * converted into a JSON string and passed as a query string argument of the
 * url.
 *
...
PhoneGap.exec = function() { 
    ...
    try {
        ...
        PhoneGap.commandQueue.push(url);
        // As long as this is the only command on the queue, we can execute it
        // immediately. If there are already commands queued, this command will
        // be kicked off by the native code after the preceding command is
        // executed.
        if (PhoneGap.commandQueue.length == 1) {
            PhoneGap.runCommand();
        }
    } catch (e) {
...
/**
 * Called by native code to signal that we're ready for the next command to
 * run.
 */
PhoneGap.finishExec = function() {
  PhoneGap.commandQueue.shift();

  if (PhoneGap.commandQueue.length != 0) {
      PhoneGap.runCommand();
  }
}
...
/**
 * Internal function used to dispatch the request to PhoneGap. It executes the
 * next command on the queue.
 * 
 * @private
 */
PhoneGap.runCommand = function() {
    if (!PhoneGap.available) {
        alert("ERROR: Attempting to call PhoneGap.exec()"
              +" before 'deviceready'. Ignoring.");
        return;
    ...
    if (!PhoneGap.execGapBridge) {
        PhoneGap.execGapBridge = PhoneGap.createGapBridge();
    }

    PhoneGap.execGapBridge.src = PhoneGap.commandQueue[0];
};

/**
 * This function executes a PhoneGap command just like PhoneGap.exec(), but
 * using a simpler and faster method. This function exists in addition to
 * PhoneGap.exec() because unlike PhoneGap.exec(), this function does not work
 * from an embedded iframe.
 *
 * Arguments for this function are the same as for PhoneGap.exec(). See the
 * documentation on PhoneGap.exec() for more information.
 */
PhoneGap.fastExec = function() {
    if (!PhoneGap.available) {
        alert("ERROR: Attempting to call PhoneGap.fastExec()"
              +" before 'deviceready'. Ignoring.");
        return;
    }
    ...
    PhoneGap.fastExecCommandQueue.push(JSON.stringify(command));
    ...
    if (PhoneGap.fastExecCommandQueue.length == 1 
        && !PhoneGap.fastExecFlushing) {
        if (!PhoneGap.fastExecGapBridge) {
            PhoneGap.fastExecGapBridge = PhoneGap.createGapBridge();
        }
        PhoneGap.fastExecGapBridge.src = "fastgap://ready";
    }
}

/**
 * Called by native code to retrieve all outstanding fastExec commands.
 */
PhoneGap.getAndClearQueuedFastExecCommands = function() {
  json = JSON.stringify(PhoneGap.fastExecCommandQueue);
  PhoneGap.fastExecCommandQueue = [];
  return json;
}
